...
function sendGeneralEmail($email, $subject, $message, $receiver_name = '')
{
    ...
    if ($config->name == 'php') {
        ...
        sendSmtpMail($config, $email, $receiver_name, $general->email_from, $general, $subject, $message);
    } else if ($config->name == 'sendgrid') {
...
{
    $general = GeneralSetting::first();
    $url = 'https://min-api.cryptocompare.com/data/price?fsym='.$coinId.'&tsyms=USD&api_key='. $general->coin_rate_api;
